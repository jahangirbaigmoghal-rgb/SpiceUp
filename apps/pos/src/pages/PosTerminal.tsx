...
export default function PosTerminal({ user, onLogout }: PosTerminalProps) {
  ...
  const isAdminUser = user && ['admin', 'manager', 'super_admin'].includes(user.role);
        ...
        <div className="flex items-center space-x-6">
          ...
          {isAdminUser && (
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-brand-600/20 border border-brand-500/30 hover:bg-brand-600/30 rounded-lg text-xs font-semibold text-brand-400 transition-colors"
            >
          ...
          <div className="p-4 border-t border-slate-900 bg-slate-950/60 space-y-2.5 shrink-0">
            {cart.length > 0 && (
              <div className="border-b border-slate-900 pb-2 space-y-1">
                ...
                {Object.values(vatBreakdown).map((bucket: any) => (
