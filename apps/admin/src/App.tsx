...
type MenuTab = 'product' | 'groups' | 'component' | 'label' | 'bundle' | 'manual-product' | 'short-hand' | 'department' | 'product-time' | 'settings';
...
interface ModifierGroup { _id: string; name: string; displayName?: string; dashboardHeading?: string; staticLabelsEnabled: boolean; allowedLabelIds?: Array<string | LabelItem>; samePrice: boolean; samePricePence: number; type: 'required' | 'optional'; selectionType: 'single' | 'multiple'; minSelections: number; maxSelections: number; options: ModifierOption[]; isActive: boolean; backgroundColor?: string; textColor?: string; }
...
interface BundleSlot { label: string; allowedCategoryIds: string[]; minChoices: number; maxChoices: number; required: boolean; }
interface BundleItem { _id: string; name: string; description?: string; bundlePricePence: number; components: BundleSlot[]; isActive: boolean; backgroundColor?: string; textColor?: string; }
...
const steps: Record<MenuTab, string[]> = {
  ...
  bundle: [
    "Create fixed-price meal deals and combos such as Pizza Meal Deal, Burger Special or Mixed Box.",
    "Add component slots and pick which categories each slot can draw from.",
    "Set the fixed bundle price and a colour so the deal stands out on POS and website.",
    "Use Manage to edit slot rules, prices or colours. Save changes when complete."
  ],
  'manual-product': [
...
const presetColors = ['#ef4444', '#f59e0b', '#22c55e', '#10b981', '#3b82f6', '#6366f1', '#a855f7', '#ec4899', '#ea580c', '#0ea5e9', '#14b8a6', '#1e293b', '#475569', '#ffffff'];
...
function titleFor(tab: MenuTab) {
  ...
  if (tab === 'bundle') return 'Bulk Deals';
...
export default function App() {
  ...
  const [bundleForm, setBundleForm] = useState<any>(null);
  ...
  async function loadAll() {
    try {
      const [catRes, depRes, compRes, labelRes, groupRes, bundleRes, productRes, timeRes, manualRes, shortRes, settingsRes] = await Promise.all([
        menuApi.categories(),
        menuApi.departments(),
        menuApi.components(),
        menuApi.labels(),
  ...
  const rows = useMemo(() => {
    ...
    if (activeTab === 'bundle') return bundles;
    ...
    return productTimes;
  }, [activeTab, categories, groups, components, labels, bundles, manualProducts, shortHands, departments, productTimes]);

  function openCreate(tab = activeTab) {
    if (tab === 'groups') return openGroup();
    if (tab === 'bundle') return openBundle();
  ...
  function openManage(row: any) {
    ...
    if (activeTab === 'bundle') return openBundle(row);
  ...
  function openBundle(bundle?: BundleItem) {
    const catIdOf = (slot: any) => (slot.allowedCategoryIds || []).map((c: any) => (typeof c === 'string' ? c : c?._id)).filter(Boolean);
    setBundleForm(bundle ? {
      name: bundle.name,
      description: bundle.description || '',
      pricePounds: toPounds(bundle.bundlePricePence || 0).toFixed(2),
      backgroundColor: bundle.backgroundColor || '#1e293b',
      textColor: bundle.textColor || '#ffffff',
      isActive: bundle.isActive !== false,
      components: (bundle.components || []).map((slot: any) => ({
        label: slot.label || '',
        allowedCategoryIds: catIdOf(slot),
        minChoices: slot.minChoices ?? 1,
        maxChoices: slot.maxChoices ?? 1,
      ...
      textColor: '#ffffff',
      isActive: true,
      components: [{ label: 'Item', allowedCategoryIds: [], minChoices: 1, maxChoices: 1, required: true }]
    });
    setDrawer({ type: 'bundle', item: bundle });
  }
  ...
  async function saveSimple(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (drawer.type === 'component') {
        const payload = { name, description: String(data.get('description') || ''), defaultPriceDeltaPence: toPence(Number(data.get('price') || 0)), color: String(data.get('backgroundColor') || '#1e293b'), textColor: String(data.get('textColor') || '#ffffff'), isActive };
        item ? await menuApi.updateComponent(item._id, payload) : await menuApi.createComponent(payload);
      }
      if (drawer.type === 'label') {
        const payload = { name, description: String(data.get('description') || ''), backgroundColor: String(data.get('backgroundColor') || '#334155'), textColor: String(data.get('textColor') || '#ffffff'), isActive };
      ...
      if (drawer.type === 'manual-product') {
        const payload = { name, code: String(data.get('code') || ''), pricePence: toPence(Number(data.get('price') || 0)), color: String(data.get('backgroundColor') || '#3b82f6'), textColor: String(data.get('textColor') || '#ffffff'), isActive };
  ...
  async function saveGroup() {
    try {
      const payload = {
        ...
        backgroundColor: groupForm.backgroundColor || '#1e293b',
        textColor: groupForm.textColor || '#ffffff',
  ...
  async function saveBundle() {
    try {
      const payload = {
        ...
        bundlePricePence: toPence(Number(bundleForm.pricePounds || 0)),
        backgroundColor: bundleForm.backgroundColor || '#1e293b',
        textColor: bundleForm.textColor || '#ffffff',
        isActive: bundleForm.isActive,
        components: (bundleForm.components || []).map((slot: any) => ({
          label: slot.label,
          allowedCategoryIds: slot.allowedCategoryIds || [],
          minChoices: Number(slot.minChoices ?? 1),
          maxChoices: Number(slot.maxChoices ?? 1),
          required: slot.required !== false
        }))
      };
      drawer?.item ? await menuApi.updateBundle(drawer.item._id, payload) : await menuApi.createBundle(payload);
      setDrawer(null);
      notify('Saved changes.');
      await loadAll();
    } catch (error: any) {
      setErrorMsg(error.response?.data?.error || 'Could not save bundle.');
    }
  }
  ...
  async function saveProduct() {
    ...
    const payload = {
      ...
      backgroundColor: productForm.backgroundColor || '#1e293b',
      textColor: productForm.textColor || '#ffffff',
  ...
  async function removeRow(row: any) {
    ...
    try {
      ...
      if (activeTab === 'bundle') await menuApi.deleteBundle(row._id);
      ...
      {drawer && (
        <RightDrawer title={drawerTitle(drawer)} onClose={() => setDrawer(null)}>
          {drawer.type === 'groups' && groupForm ? <GroupForm form={groupForm} setForm={setGroupForm} labels={labels} components={components} onSave={saveGroup} /> :
            drawer.type === 'bundle' && bundleForm ? <BundleForm form={bundleForm} setForm={setBundleForm} categories={categories} onSave={saveBundle} /> :
...
function drawerTitle(drawer: DrawerMode) {
  ...
  if (drawer.type === 'bundle') return drawer.item ? 'Edit Deal' : 'Add Deal';
...
/**
 * Reusable color picker: preset swatches + a native color input for custom
 * shades, plus a live preview chip. Used across categories, sub-categories,
 * products, components, labels, modifier groups and bundles.
 */
function ColorPicker({
  ...
  const resolved = value || '#1e293b';
  const isPreset = presetColors.includes(resolved.toLowerCase());
  return (
    <Field label={label}>
      <div className="space-y-2.5">
        <div className="flex flex-wrap gap-1.5">
          {presetColors.map(color => {
            const selected = resolved.toLowerCase() === color.toLowerCase();
            return (
              <button
                key={color}
                type="button"
                onClick={() => onChange(color)}
                className={`h-7 w-7 rounded-md border-2 transition-transform ${selected ? 'border-slate-900 scale-110 shadow' : 'border-slate-200 hover:scale-105'}`}
        ...
        {/* Text color toggle (light/dark) + live preview */}
        <div className="flex items-center justify-between gap-3">
          {onTextColorChange ? (
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-500">Text</span>
              {['#ffffff', '#0f172a'].map(tc => (
                <button
                  key={tc}
                  type="button"
                  onClick={() => onTextColorChange(tc)}
                  className={`h-6 w-6 rounded-full border-2 ${(textColor || '#ffffff').toLowerCase() === tc ? 'border-slate-900' : 'border-slate-200'}`}
          ...
          <span
            ...
            style={{ backgroundColor: resolved, color: textColor || (isPreset && resolved.toLowerCase() === '#ffffff' ? '#0f172a' : '#ffffff') }}
          >
            Preview
          </span>
        </div>
...
function BundleForm({ form, setForm, categories, onSave }: { form: any; setForm: any; categories: Category[]; onSave: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <Field label="Deal Name"><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className={inputClass} /></Field>
      <Field label="Description"><textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className={`${inputClass} h-16`} /></Field>
      <Field label="Fixed Deal Price (£)"><input type="number" step="0.01" value={form.pricePounds} onChange={e => setForm({ ...form, pricePounds: e.target.value })} className={inputClass} /></Field>
      <ColorPicker
        label="Deal Button Color"
        value={form.backgroundColor}
        onChange={(c) => setForm({ ...form, backgroundColor: c })}
        textColor={form.textColor}
        onTextColorChange={(c) => setForm({ ...form, textColor: c })}
      />
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="font-semibold">Component Slots</p>
          <button onClick={() => setForm({ ...form, components: [...form.components, { label: 'Item', allowedCategoryIds: [], minChoices: 1, maxChoices: 1, required: true }] })} className="text-slate-700 hover:text-brand-500"><Plus className="h-5 w-5" /></button>
        </div>
        {form.components.map((slot: BundleSlot, index: number) => (
          <div key={index} className="space-y-2 rounded border border-slate-200 p-3">
            <div className="flex items-center gap-2">
              <input value={slot.label} onChange={e => updateSlot(index, { label: e.target.value })} className={inputClass} placeholder="Slot label (e.g. Pizza 1)" />
              <button onClick={() => setForm({ ...form, components: form.components.filter((_: any, i: number) => i !== index) })} className="text-slate-400 hover:text-rose-500"><X className="h-4 w-4" /></button>
            </div>
            <Field label="Allowed Categories">
              <div className="flex flex-wrap gap-1.5">
                {categories.map(c => {
                  const selected = slot.allowedCategoryIds.includes(c._id);
                  return (
                    <button
                      key={c._id}
                      type="button"
                      onClick={() => updateSlot(index, { allowedCategoryIds: selected ? slot.allowedCategoryIds.filter((id: string) => id !== c._id) : [...slot.allowedCategoryIds, c._id] })}
                      className={`rounded px-2 py-1 text-xs font-medium ${selected ? 'bg-brand-500 text-white' : 'bg-slate-100 text-slate-600'}`}
                    >
                      {c.name}
                    </button>
                  );
            ...
            <div className="grid grid-cols-3 gap-2">
              <Field label="Min"><input type="number" value={slot.minChoices} onChange={e => updateSlot(index, { minChoices: Number(e.target.value) })} className={inputClass} /></Field>
              <Field label="Max"><input type="number" value={slot.maxChoices} onChange={e => updateSlot(index, { maxChoices: Number(e.target.value) })} className={inputClass} /></Field>
              <label className="flex items-end gap-1 pb-2 text-xs"><input type="checkbox" checked={slot.required} onChange={e => updateSlot(index, { required: e.target.checked })} /> Required</label>
            </div>
          </div>
        ))}
      </div>
      <label className="flex items-center justify-between rounded border border-slate-200 p-3 text-sm"><span>Active</span><input type="checkbox" checked={form.isActive} onChange={e => setForm({ ...form, isActive: e.target.checked })} /></label>
...
function SimpleForm({ drawer, departments, products, onSubmit }: { drawer: DrawerMode; departments: Department[]; products: any[]; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  const item = drawer?.item || {};
  const hasColor = drawer?.type === 'product' || drawer?.type === 'component' || drawer?.type === 'label' || drawer?.type === 'manual-product';
  const [bgColor, setBgColor] = useState((item.backgroundColor || item.color || '#f59e0b') as string);
  const [txtColor, setTxtColor] = useState((item.textColor || '#ffffff') as string);
  return (
      ...
      {hasColor && (
        <>
          <ColorPicker
            ...
            onChange={setBgColor}
            textColor={txtColor}
            onTextColorChange={setTxtColor}
          />
          {/* Hidden fields so FormData submit picks up the picker values */}
          <input type="hidden" name="backgroundColor" value={bgColor} />
          <input type="hidden" name="textColor" value={txtColor} />
        </>
      )}
...
function GroupForm({ form, setForm, labels, components, onSave }: { form: any; setForm: any; labels: LabelItem[]; components: ComponentItem[]; onSave: () => void }) {
  return (
    <div className="space-y-4">
      ...
      <ColorPicker
        label="Group Button Color"
        value={form.backgroundColor || '#1e293b'}
        onChange={(c) => setForm({ ...form, backgroundColor: c })}
