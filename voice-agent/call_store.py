"""Unified call recording ingestion service."""
...
from s3_store import is_s3_configured, upload_to_s3
...
DEFAULT_ORG_ID = ObjectId("000000000000000000000001")
...
def save_call_record(
    ...
    *,
    ...
    """Unified ingestion function for all calls (WebSocket and LiveKit)."""
    if db is None:
    ...
    # Determine organization scoping and retention
    org_id = DEFAULT_ORG_ID
    retention_days = 90
    if profile:
        # Extract organizationId (try root level first, then config)
        raw_org_id = profile.get("organizationId") or profile.get("config", {}).get("organizationId")
        if raw_org_id:
            try:
                org_id = ObjectId(raw_org_id) if not isinstance(raw_org_id, ObjectId) else raw_org_id
            except Exception as e:
                logger.error("Failed to parse organizationId %s: %s", raw_org_id, e)
        
        # Extract retention configuration
        retention_days = profile.get("recordingRetentionDays") or profile.get("config", {}).get("recordingRetentionDays", 90)

    retention_expires_at = (started_at or datetime.now(timezone.utc)) + timedelta(days=int(retention_days))

    # Initialize audio file IDs as None
    user_file_id = None
    agent_file_id = None
    stereo_file_id = None
    use_s3 = is_s3_configured()
    storage_type = "s3" if use_s3 else "gridfs"

    # Process and upload audio if provided
    if user_wav_path and agent_wav_path:
        try:
            ...
            if use_s3:
                # S3 Object Keys
                user_key = f"recordings/{call_sid}/user.wav"
                agent_key = f"recordings/{call_sid}/agent.wav"
                stereo_key = f"recordings/{call_sid}/stereo.wav"

                if upload_to_s3(user_data, user_key, "audio/wav"):
                    user_file_id = user_key
                if upload_to_s3(agent_data, agent_key, "audio/wav"):
                    agent_file_id = agent_key
                if upload_to_s3(stereo_data, stereo_key, "audio/wav"):
                    stereo_file_id = stereo_key
            else:
                # GridFS Bucket Upload
                fs = GridFS(db, collection="callRecordingFiles")
                user_file_id = fs.put(user_data, filename=f"user_{call_sid}.wav", content_type="audio/wav")
                agent_file_id = fs.put(agent_data, filename=f"agent_{call_sid}.wav", content_type="audio/wav")
                stereo_file_id = fs.put(stereo_data, filename=f"stereo_{call_sid}.wav", content_type="audio/wav")

        except FileNotFoundError as e:
            logger.error("Recording file missing for %s: %s", call_sid, e)
        except Exception as e:
            logger.error("Error processing/uploading audio for %s: %s", call_sid, e)
        ...
        try:
            ...
            if not contact:
                contact_doc = {
                    ...
                    "email": "",
                    "company": "",
                    "notes": f"Created automatically from {voice_gateway_provider} call {call_sid}.",
    ...
    if voice_gateway_provider == "livekit":
        # LiveKit costs
        livekit_cost = round(billable_minutes * 0.005, 4)
        twilio_stream_cost = 0.0
        twilio_cost = twilio_voice_cost
        gemini_input_cost = 0.0
        gemini_output_cost = 0.0
        gemini_cost = round(billable_minutes * 0.002, 4)
        total_cost = round(twilio_voice_cost + livekit_cost + gemini_cost, 4)
    else:
        # WebSocket bridge costs
        livekit_cost = 0.0
    ...
    doc = {
        ...
        # Audio File references (String if S3, ObjectId if GridFS)
        ...
        "livekitCost": livekit_cost,
        "latencyData": latency_data,
        "actionItems": action_items,
        "crucialEvents": crucial_events,
        "transcript": transcript_timeline,
        "voiceGatewayProvider": voice_gateway_provider,
        
        "createdAt": datetime.now(timezone.utc),
    }

    result = db.callRecordings.insert_one(doc)
    logger.info("Saved %s call recording %s to MongoDB (_id=%s) with total cost $%s, storageType=%s", 
                voice_gateway_provider, call_sid, result.inserted_id, total_cost, storage_type)
