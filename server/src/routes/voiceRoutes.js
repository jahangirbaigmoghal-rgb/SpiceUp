...
voiceRoutes.get('/fix-model-temp', async (req, res) => {
  try {
    ...
    const resultSettings = await db.collection('settings').updateMany({}, { $set: { voiceAgentModel: 'gemini-2.0-flash-exp' } });
    const resultTenants = await db.collection('tenants').updateMany({}, { $set: { voiceAgentModel: 'gemini-2.0-flash-exp' } });
